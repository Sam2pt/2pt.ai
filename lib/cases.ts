...
const ALL_CASES: CaseStudy[] = [
  {
    ...
    approach:
      "We deployed a single central customer brain for the whole portfolio and wrapped it in per-brand skins so every company opens their own tenant and sees a product that feels native to them. Same brain, different chrome, different voice. On top of that we built per-role framings so the same data lands differently for a marketer, a founder and a board member. The depth of insight on each brand's own customer went well past anything the brands had had before. Cohort behaviour, LTV shape, funnel drop-off, creative response, retention triggers, all in one place, all live. Every tenant can spin experiments (cohort, price, creative, funnel) and choose per experiment whether to keep the learning private or share it into the portfolio pool. When a shared experiment lands, the brain matches the audience shape against every other tenant and surfaces the winning play as a suggestion in the brands where it is likely to work. The data never travels. Only the plays do, and only the ones a brand chose to share. Rolled out to seven brands in eight weeks with embedded engineers alongside each brand's growth lead.",
    system:
      "One central customer brain. Seven skinned tenants. Integrated with each brand's commerce, CDP, ad and support stack. Cross-brand suggestion layer runs continuously with per-experiment opt-in. Each brand owns their tenant post-handover.",
    outcomes: [
